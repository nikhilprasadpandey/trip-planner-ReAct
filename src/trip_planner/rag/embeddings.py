...
text-embedding-3-small. This is the one non-Anthropic API call in the
system; every agent's reasoning stays on Claude (see cost/ledger.py, which
tracks this separately from LLM token cost — see config/model_prices.yaml
`embeddings:`).

Swappable by design (same pattern as the flight provider): if you need to
change providers later, keep this module's function signatures and swap the
implementation.
"""
