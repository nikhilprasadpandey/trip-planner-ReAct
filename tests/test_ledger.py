...
def test_record_llm_usage_computes_cost_from_model_prices():
    cost = ledger.record_llm_usage(
        "trace-1", agent="WeatherAgent", model="claude-sonnet-5",
        input_tokens=1000, output_tokens=500, cache_read_tokens=0,
    )
    # sonnet-5: $2.00/1M input, $10.00/1M output
...
def test_cached_input_tokens_billed_at_cache_read_rate_not_double_counted():
    cost = ledger.record_llm_usage(
        "trace-2", agent="PolicyAgent", model="claude-sonnet-5",
        input_tokens=1000, output_tokens=0, cache_read_tokens=800,
    )
    # 200 uncached @ $2.00/1M + 800 cached @ $0.20/1M
    expected = (200 * 2.00 + 800 * 0.20) / 1_000_000
    assert cost == pytest.approx(expected)


def test_multiple_calls_accumulate_agent_cost_for_same_trace():
    ledger.record_llm_usage("trace-3", agent="WeatherAgent", model="claude-sonnet-5", input_tokens=100, output_tokens=100)
    ledger.record_llm_usage("trace-3", agent="FlightAgent", model="claude-sonnet-5", input_tokens=100, output_tokens=100)
...
def test_business_cost_tracked_separately_from_agent_cost():
    ledger.record_llm_usage("trace-4", agent="FlightAgent", model="claude-sonnet-5", input_tokens=100, output_tokens=100)
...
def test_unknown_model_raises():
    with pytest.raises(ValueError):
        ledger.record_llm_usage("trace-5", agent="X", model="gpt-not-a-claude-model", input_tokens=10, output_tokens=10)
